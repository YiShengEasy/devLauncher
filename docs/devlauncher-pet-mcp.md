...
```bash
DEVLAUNCHER_PET_MCP_INBOX=/absolute/path/pet-mcp-events.jsonl node /path/to/devLauncher/mcp/devlauncher-pet-mcp.mjs
```
...
```bash
node /path/to/devLauncher/scripts/pet-status.mjs working "正在处理：测试 MCP"
```
